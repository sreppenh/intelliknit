...
const KnittingStepCounter = ({
    ...
    const instructionResult = getInstructionWithColor();
    ...
    const getProgressInfoText = () => {
        if (isLengthStep && lengthDisplayData) {
            // For distance-based steps, show both stitch count and target length
            const lengthTarget = getLengthTarget(step);
            if (lengthTarget && targetStitches) {
                return `Target: ${targetStitches} stitches (knit to ${lengthTarget.value} ${lengthTarget.units})`;
            }
            ...
            <div className="text-center px-6 relative z-10 w-full max-w-sm">
                <div className="knitting-content-sage backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/50 mb-6">
                    ...
                    <div className={`${instructionResult.isBrioche ? 'text-left' : ''} mb-4`}>
                        {/* Main instruction - primary focus */}
                        ...
                        {instructionResult.colorInfo && (
                            <div className={`text-sm ${theme.textSecondary} flex justify-center`}>
                                {instructionResult.colorInfo.jsx}
                            </div>
                        )}
                    </div>
