...
/**
 * Generate detailed marled stripe sequence display
 */
const getMarledStripeSequenceDisplay = (step, project) => {
    const colorwork = step.colorwork || step.wizardConfig?.colorwork || step.advancedWizardConfig?.colorwork;
    ...
    // Get color mapping from project
    const getColorName = (letter) => {
        const yarn = project?.yarns?.find(y => y.letter === letter);
        if (yarn?.color) {
            return yarn.color;
    ...
    // Format each marled stripe in the sequence
    const marledDescriptions = marledSequence.map(marled => {
        const color1Name = getColorName(marled.colors[0]);
        const color2Name = getColorName(marled.colors[1]);
        const rowCount = marled.rows || 1;
        const term = rowCount === 1 ? rowTermSingle : rowTerm;
        return `${rowCount} ${term} ${color1Name}+${color2Name}`;
    });

    return marledDescriptions.join(', ');
};
...
export const getContextualColorNotes = (step, project = null) => {
    ...
    if (colorwork?.type === 'marled_stripes') {
        return getMarledStripeSequenceDisplay(step, project);
    }
    ...
    if (colorwork) {
        ...
        if (colorwork.type === 'marled_stripes') {
            ...
            if (marledSequence && marledSequence.length > 0) {
                const uniqueColors = [...new Set(marledSequence.flatMap(m => m.colors))];
                const colorNames = uniqueColors.map(letter => {
                    const yarn = getYarnByLetter(project?.yarns || [], letter);
                    return `Color ${yarn.letter}`;
                }).join(', ');
