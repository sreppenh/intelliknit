...
export const getColorSequenceLength = (step) => {
    ...
    if (colorwork.type === 'marled_stripes') {
        ...
        if (!marledSequence || !Array.isArray(marledSequence) || marledSequence.length === 0) {
            return null;
        }
        return marledSequence.reduce((sum, marled) => sum + (marled.rows || 0), 0);
    }
...
export const calculateContinuationState = (step, rowsKnitted = null) => {
    ...
    if (colorLength && colorLength > 0) {
        continuation.colorRow = totalRows % colorLength;
...
export const getColorRowOffset = (step, component, stepIndex, projectId) => {
    ...
    // Both must be the same type of color pattern
    if (currentColorwork?.type !== prevColorwork?.type) {
        return 0;
    }

    // For regular stripes, compare stripe sequences
    if (currentColorwork.type === 'stripes') {
    ...
    // For marled stripes, compare marled sequences
    if (currentColorwork.type === 'marled_stripes') {
        if (!doMarledSequencesMatch(currentColorwork.marledSequence, prevColorwork.marledSequence)) {
            console.warn('Marled stripe sequence changed - resetting color continuation');
            return 0;
        }
    }
...
/**
 * Helper: Compare two marled stripe sequences for exact match
 */
function doMarledSequencesMatch(seq1, seq2) {
    if (!seq1 || !seq2) return false;
    if (seq1.length !== seq2.length) return false;

    for (let i = 0; i < seq1.length; i++) {
        if (seq1[i].rows !== seq2[i].rows) return false;
        if (seq1[i].colors.length !== seq2[i].colors.length) return false;
        for (let j = 0; j < seq1[i].colors.length; j++) {
            if (seq1[i].colors[j] !== seq2[i].colors[j]) return false;
        }
    }
